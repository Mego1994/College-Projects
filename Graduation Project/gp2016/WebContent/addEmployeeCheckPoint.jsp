...
<%
	...
	if(rs2.next() == false){
		if(fname == "" || lname == "" || bdate == "" || email == "" || username == "" || password == "" || gender == "" || address == "" || phone == "" || jobStartDate == "")
			response.sendRedirect("addEmployeeFillAllFields.jsp");
		else{	
			statement.executeUpdate("insert into emloyee2 values ('"+username+"','"+fname+"','"+lname+"','"+phone+"','"+email+"','"+address+"','"+password+"','"+gender+"','"+bdate+"','"+jobStartDate+"', '"+"active"+"')");
			statement.executeUpdate("insert into roles values ('"+username+"','"+password+"','"+"receptionist"+"','"+phone+"', '"+"active"+"' ,'"+"employee"+"')");
			response.sendRedirect("addedmsgemp.jsp");
		}
