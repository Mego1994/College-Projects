...
if (request.getParameter("mail") != null) {
		...
		ResultSet rs1 = statement.executeQuery("select * from insertpatient where patientID = '" + session.getAttribute("PatientID") + "'");
		...
		while (rs1.next()) {
			...
			rs1 = statement.executeQuery("select * from insertpatient where patientID = '" + session.getAttribute("PatientID") + "'");

			while (rs1.next()) {

			cell = new PdfPCell(new Phrase((String) rs1.getString("username")));
			cell.setBorder(Rectangle.NO_BORDER);
			patientTable.addCell(cell);
		...
		while(rs1.next())
		{
			if(session.getAttribute("PatientID").equals(rs1.getString("patientID")))
			{
		...
		while (i < testsCommaSplit.length) {
			...
			rs1 = statement.executeQuery("select * from samples where patientID = '" + session.getAttribute("PatientID") + "'");
			if (rs1.next()) {
...
// 				testTable.addCell(cell2);
				...
				cell2 = new PdfPCell(new Phrase((String) session.getAttribute("patientdate")));
				cell2.setBorder(Rectangle.NO_BORDER);
				testTable.addCell(cell2);
