...
if (request.getParameter("mail") != null) {
		...
		ResultSet rs1 = statement.executeQuery("select * from insertpatient where patientID = '" + session.getAttribute("caseWordRemoved") + "'");
		String to;
		...
		while (rs1.next()) {
			...
			rs1 = statement.executeQuery("select * from insertpatient where patientID = '" + session.getAttribute("caseWordRemoved") + "'");
		...
		while(rs1.next())
		{
			if(session.getAttribute("caseWordRemoved").equals(rs1.getString("patientID")))
			{
				String tests = rs1.getString("tests");
				String dates = rs1.getString("visitDate");
...
// 				testTable.addCell(cell2);
				...
				rs1 = statement.executeQuery("select * from samples where patientID = '" + session.getAttribute("caseWordRemoved") + "'");
				if (rs1.next()) {
